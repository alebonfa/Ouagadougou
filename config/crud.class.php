	...
	class crud
	{
		...
		public function inserir($campos, $valores) {
			...
			if(!$this->ins = mysql_query($this->sql_ins)) {
				die("<center>Erro na inclusão (III)" . "<br>Linha: " . __LINE__ . "<br>" . mysql_error() . 
		...
		public function atualizar($camposvalores, $where=NULL) {
			...
			if(!$this->upd = mysql_query($this->sql_upd)) {
				die('<center>Erro na atualização ' . '<br>Linha: ' . __LINE__ . '<br>' . mysql_error() . 
				'<br> <a href="index.php">Voltar ao Menu</a> </center>' );
			} else {
		...
		public function excluir($where = NULL) {
			...
			echo "<script>alert($this->sql_del)</script>";
			...
			if($regs>0) {
				if(!$this->del = mysql_query($this->sql_del)) {
					die('<center>Erro na exclusão ' . '<br>Linha: ' . __LINE__ . '<br>' . mysql_error() .
					'<br> <a href="index.php">Voltar ao Menu</a> </center>' );
				} else {
					print '<center>Registro Excluido com Sucesso!<br><a href="index.php">Voltar ao Menu</a></center>';
				}
			} else {
				print '<center>Registro nao Encontrado!<br><a href="index.php">Voltar ao Menu</a></center>';
			}
		}
